...
from model.grid_cross5_model_S1_deng import model_S0
...
def train(dim, l, head, model, train_data_loader, index, test_data_loader, loss_fn, optimizer, n_epochs, device,
          scheduler=None):
    print('Starting training at', datetime.today())

    # src, dst, rel, hr2eid, rt2eid = construct_graph(index)
    # traingraph,_ = construct_trans_graph(index)#get_kg(src, dst, rel, device)
    ...
    for i in range(1, n_epochs + 1):
        ...
        with torch.no_grad():
            ...
            train_acc, train_f1, train_recall, train_precision, kappa, _, _ = do_compute_metrics(train_probas_pred,
                                                                                                 train_ground_truth)
            # if best_f1 < train_f1:
            #     with open('S2_gridsearch_deng_trainresults.csv', 'a') as f:
            ...
            test_acc, test_f1, test_recall, test_precision, test_kappa, predlabls, truelabels = do_compute_metrics(
    ...
    def run_model(dim, l, head):  # params
        ...
        for index in range(1, 6):
            ...
            df_ddi_train = pd.read_csv(
                '../data/Dengcross5datasets/Deng_S1/S1_DDIMDL_idx_train_cross_' + str(index) + '.csv')
            ...
            df_ddi_test = pd.read_csv(
                '../data/Dengcross5datasets/Deng_S1/S1_DDIMDL_idx_test_cross_' + str(index) + '.csv')
            ...
            train(dim, l, head, model, train_data_loader, index, test_data_loader, loss, optimizer, n_epochs, device,
                  scheduler)
