...
def mean_pool_nodes(node_feats, batch, num_graphs=None):
    """
    node_feats: (N, D) 节点特征
    batch: (N,) 每个节点所属的图索引 [0 ... B-1]
    num_graphs: 图的数量（可选），默认用 batch.max()+1 推断
    返回: (B, D) 图级表示
    """
...
class model_S0(torch.nn.Module):
    ...
    def forward(self, drug1s, drug2s):
        ...
        # print(self.alignn.shape)
        #drug1_emb_3dmol = self.dmol[drug1s.long()]
        #drug2_emb_3dmol = self.dmol[drug2s.long()]
