...
class model_S0(torch.nn.Module):
    def __init__(self,device,index,dim,l,head):#in_dim,hidden_dim,out_dim,
        ...
        self.mlp = nn.ModuleList([nn.Linear(3072,3072),#
                                  nn.ELU(),
                                  nn.Linear(3072, 86)
                                  ])
    ...
    def forward(self, drug1s, drug2s, train_graph):
        ...
        alignn = self.bn1(self.alignn)
        drug1_emb_graph = alignn[drug1s.long()]
        drug2_emb_graph = alignn[drug2s.long()]
        ...
        all = torch.cat(
            [drug1_emb_morgen, drug1_emb_graph,drug1_3d_emb, drug2_emb_morgen, drug2_emb_graph,drug2_3d_emb],
            dim=1)  # [B, 12D]
        # print(all.shape)
