...
class DDI_GraphTransformer(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, heads, num_layers):
        ...
        #self.conv2 = TransformerConv(hidden_channels * heads, hidden_channels, heads, dropout=0.1)
...
def mean_pool_nodes(
    node_feats: torch.Tensor,   # [N, F] 或 [N]
    batch: torch.Tensor,        # [N]，每个节点的图ID（0..B-1）
    num_graphs: Optional[int] = None,
    normalize: str = 'sum',
) -> torch.Tensor:
    # 形状与类型规范
    if node_feats.dim() == 1:
        node_feats = node_feats.unsqueeze(1)           # [N] -> [N,1]
    assert batch.numel() == node_feats.size(0), "batch 与 node_feats 行数不一致"
    ...
    # 按图分组求和
    B, F = num_graphs, node_feats.size(1)
    out = node_feats.new_zeros(B, F)                   # 继承 dtype/device
    out.index_add_(0, batch, node_feats)               # group-sum

    if normalize == 'sum':
        return out
    # 计数并做归一化
    counts = torch.bincount(batch, minlength=B).to(node_feats.device)  # [B]
    if normalize == 'mean':
        denom = counts.clamp_min(1).unsqueeze(1).to(node_feats.dtype)  # 避免除零
        return out / denom
    # L2 归一化到单位范数
    norms = out.norm(p=2, dim=1, keepdim=True).clamp_min(1e-12)
    return out / norms
...
class model_S0(torch.nn.Module):
    def __init__(self,device,index,dim,l,head):#in_dim,hidden_dim,out_dim,
        ...
        self.trans1 = DDI_GraphTransformer(768, 128, 2, 2)

        self.mlp = nn.ModuleList([nn.Linear(4096,4096),#2560
                                  nn.ELU(),
                                  nn.Linear(4096, 86)
                                  ])
        ...
        alignn_path = '../data/drugfeatures/drugs1559_mol86.npy'  #
        alignn = np.load(alignn_path)
        self.alignn = torch.tensor(alignn).to(device).float()

        with open("../data/drugfeatures/RYU_drug_3dproperty_1559.pkl", "rb") as f:
        ...
        self.ae1=AE1(1024,self.dim*head*2,self.head)#
        self.egnn = EGNN(
        ...
        self.cl_lambda = 0.1  # 分类损失的权重系数 λ，可调 0.05–0.2

        # 三个模态的投影头（drug1/2 复用同一组权重，以实现“同模态共享空间”）
        # 若你的环境没有 LazyLinear，请改为 nn.Linear(in_dim, self.cl_proj_dim)
    ...
    def forward(self, drug1s, drug2s):
        ...
        drug1_3d_emb = self.bn(drug1_3d_emb)
        ...
        drug2_3d_emb = self.bn(drug2_3d_emb)

        all = torch.cat(
            [drug1_emb_morgen, drug1_emb_graph, drug2_emb_morgen,drug1_3d_emb, drug2_emb_graph,drug2_3d_emb],
            dim=1)  # [B, 12D]
        # print(all.shape)
        ...
        # 2) 同一“药物实体”的不同模态对齐（drug1 内部一对一，drug2 内部一对一）
        #    你当前 batch 内，drug1s 与 drug2s 的索引分别独立成对齐关系，
        #    因为每一条样本的三模态是一一对应的正样本，其它样本为负样本。
        cl_loss_d1 = (
        ...
        #cl_loss_d1 = self._info_nce(z1_m, z1_g, self.cl_temp)
        #cl_loss_d2 = self._info_nce(z2_m, z2_g, self.cl_temp)
        cl_loss = 1 * (cl_loss_d1 + cl_loss_d2)
